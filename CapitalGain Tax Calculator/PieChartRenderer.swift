...
public class PieChartRenderer: ChartDataRendererBase
{
        ...
        for (var j = 0; j < entries.count; j++)
        {
            ...
            if ((abs(e.value) > 0.000001))
            {
                    ...
                    dataSetIndex: _chart.data!.indexOfDataSet(dataSet)))
                {
                    ...
                    CGPathAddArc(path, nil, circleBox.midX, circleBox.midY, radius, startAngle * ChartUtils_Pie.Math.FDEG2RAD, endAngle * ChartUtils_Pie.Math.FDEG2RAD, false)
                    CGPathCloseSubpath(path)
                    
                    if (innerRadius > 0.0)
                    {
                        CGPathMoveToPoint(path, nil, circleBox.midX, circleBox.midY)
                        CGPathAddArc(path, nil, circleBox.midX, circleBox.midY, innerRadius, startAngle * ChartUtils_Pie.Math.FDEG2RAD, endAngle * ChartUtils_Pie.Math.FDEG2RAD, false)
                        CGPathCloseSubpath(path)
                    }
    ...
    public override func drawValues(context context: CGContext)
    {
        ...
        for (var i = 0; i < dataSets.count; i++)
        {
            ...
            for (var j = 0, maxEntry = Int(min(ceil(CGFloat(entries.count) * _animator.phaseX), CGFloat(entries.count))); j < maxEntry; j++)
            {
                ...
                let x = (r * cos(((rotationAngle + absoluteAngles[cnt] - offset) * _animator.phaseY) * ChartUtils_Pie.Math.FDEG2RAD) + center.x)
                var y = (r * sin(((rotationAngle + absoluteAngles[cnt] - offset) * _animator.phaseY) * ChartUtils_Pie.Math.FDEG2RAD) + center.y)
                ...
                {
                    ChartUtils_Pie.drawText(context: context, text: val, point: CGPoint(x: x, y: y), align: .Center, attributes: [NSFontAttributeName: valueFont, NSForegroundColorAttributeName: valueTextColor])
                    
                    if (j < data.xValCount && data.xVals[j] != nil)
                    {
                        ChartUtils_Pie.drawText(context: context, text: data.xVals[j]!, point: CGPoint(x: x, y: y + lineHeight), align: .Center, attributes: [NSFontAttributeName: valueFont, NSForegroundColorAttributeName: valueTextColor])
                    }
                }
                else if (drawXVals && !drawYVals)
                {
                    ChartUtils_Pie.drawText(context: context, text: data.xVals[j]!, point: CGPoint(x: x, y: y + lineHeight / 2.0), align: .Center, attributes: [NSFontAttributeName: valueFont, NSForegroundColorAttributeName: valueTextColor])
                }
                else if (!drawXVals && drawYVals)
                {
                    ChartUtils_Pie.drawText(context: context, text: val, point: CGPoint(x: x, y: y + lineHeight / 2.0), align: .Center, attributes: [NSFontAttributeName: valueFont, NSForegroundColorAttributeName: valueTextColor])
                }
    ...
    public override func drawHighlighted(context context: CGContext, indices: [ChartHighlight])
    {
        ...
        for (var i = 0; i < indices.count; i++)
        {
            ...
            CGPathAddArc(path, nil, highlighted.midX, highlighted.midY, highlighted.size.width / 2.0, startAngle * ChartUtils_Pie.Math.FDEG2RAD, endAngle * ChartUtils_Pie.Math.FDEG2RAD, false)
            CGPathCloseSubpath(path)
            
            if (innerRadius > 0.0)
            {
                CGPathMoveToPoint(path, nil, highlighted.midX, highlighted.midY)
                CGPathAddArc(path, nil, highlighted.midX, highlighted.midY, innerRadius, startAngle * ChartUtils_Pie.Math.FDEG2RAD, endAngle * ChartUtils_Pie.Math.FDEG2RAD, false)
                CGPathCloseSubpath(path)
            }
