...
public class PieChartView: PieRadarChartViewBase
{
    ...
    public override func getMarkerPosition(entry e: ChartDataEntry, highlight: ChartHighlight) -> CGPoint
    {
        ...
        let x: CGFloat = (r * cos(((rotationAngle + absoluteAngles[i] - offset) * _animator.phaseY) * ChartUtils_Pie.Math.FDEG2RAD) + center.x)
        let y: CGFloat = (r * sin(((rotationAngle + absoluteAngles[i] - offset) * _animator.phaseY) * ChartUtils_Pie.Math.FDEG2RAD) + center.y)
    ...
    public override func indexForAngle(angle: CGFloat) -> Int
    {
        ...
        let a = ChartUtils_Pie.normalizedAngleFromAngle(angle - self.rotationAngle)
