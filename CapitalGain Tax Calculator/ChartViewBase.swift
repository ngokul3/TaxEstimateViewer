...
public class ChartViewBase: UIView, ChartAnimatorDelegate
{
    ...
    public var descriptionText = "Capital Gain @ Tax %"
    ...
    internal func calculateFormatter(#min: Double, max: Double)
    {
        ...
        var digits = ChartUtils_Pie.decimals(reference);
    ...
    public override func drawRect(rect: CGRect)
    {
        ...
        { // check if there is data
            ...
            ChartUtils_Pie.drawText(context: context, text: noDataText, point: CGPoint(x: frame.width / 2.0, y: frame.height / 2.0), align: .Center, attributes: [NSFontAttributeName: infoFont, NSForegroundColorAttributeName: infoTextColor]);
            
            if (noDataTextDescription != nil && count(noDataTextDescription!) > 0)
            {
                var textOffset = -infoFont.lineHeight / 2.0;
                
                ChartUtils_Pie.drawText(context: context, text: noDataTextDescription!, point: CGPoint(x: frame.width / 2.0, y: frame.height / 2.0 + textOffset), align: .Center, attributes: [NSFontAttributeName: infoFont, NSForegroundColorAttributeName: infoTextColor]);
            }
    ...
    internal func drawDescription(#context: CGContext)
    {
        ...
        ChartUtils_Pie.drawText(context: context, text: descriptionText, point: CGPoint(x: frame.width - _viewPortHandler.offsetRight - 10.0, y: frame.height - _viewPortHandler.offsetBottom - 10.0 - font!.lineHeight), align: .Right, attributes: attrs);
    }
