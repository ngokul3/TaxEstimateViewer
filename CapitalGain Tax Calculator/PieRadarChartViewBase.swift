...
public class PieRadarChartViewBase: ChartViewBase
{
    ...
    public func angleForPoint(#x: CGFloat, y: CGFloat) -> CGFloat
    {
        ...
        var angle = r * ChartUtils_Pie.Math.RAD2DEG;
    ...
    {
        var a = cos(angle * ChartUtils_Pie.Math.FDEG2RAD);
        return CGPoint(x: center.x + dist * cos(angle * ChartUtils_Pie.Math.FDEG2RAD),
            y: center.y + dist * sin(angle * ChartUtils_Pie.Math.FDEG2RAD));
    }
    ...
    public var rotationAngle: CGFloat
        ...
        set
        {
            ...
            _rotationAngle = ChartUtils_Pie.normalizedAngleFromAngle(newValue);
            setNeedsDisplay();
        }
    }
        ...
        if (recognizer.state == UIGestureRecognizerState.Ended)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (self.isKindOfClass(RadarChartView))
                    {
                        dataSetIndex = ChartUtils_Pie.closestDataSetIndex(valsAtIndex, value: Double(distance / (self as! RadarChartView).factor), axis: nil);
                    }
    ...
    @objc private func rotationGestureRecognized(recognizer: UIRotationGestureRecognizer)
    {
        ...
        {
            var angle = ChartUtils_Pie.Math.FRAD2DEG * recognizer.rotation;
        ...
        else if (recognizer.state == UIGestureRecognizerState.Ended)
        {
            ...
            if (isDragDecelerationEnabled)
            {
                ...
                _decelerationAngularVelocity = ChartUtils_Pie.Math.FRAD2DEG * recognizer.velocity;
