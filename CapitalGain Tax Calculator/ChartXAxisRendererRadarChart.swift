...
public class ChartXAxisRendererRadarChart: ChartXAxisRenderer
{
    ...
    public override func renderAxisLabels(#context: CGContext)
    {
        ...
        for (var i = 0, count = _xAxis.values.count; i < count; i++)
        {
            ...
            var p = ChartUtils_Pie.getPosition(center: center, dist: CGFloat(_chart.yRange) * factor + _xAxis.labelWidth / 2.0, angle: angle);
            
            ChartUtils_Pie.drawText(context: context, text: text!, point: CGPoint(x: p.x, y: p.y - _xAxis.labelHeight / 2.0), align: .Center, attributes: [NSFontAttributeName: labelFont, NSForegroundColorAttributeName: labelTextColor]);
        }
    }
