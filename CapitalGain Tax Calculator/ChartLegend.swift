...
public class ChartLegend: ChartComponentBase
{
    ...
    /// colors that will be appended to the end of the colors array after calculating the legend.
    public var extraColorsObjc: [NSObject] { return ChartUtils_Pie.bridgedObjCGetUIColorArray(swift: _extraColors); }
    
    /// labels that will be appended to the end of the labels array after calculating the legend. a nil label will start a group.
    public var extraLabelsObjc: [NSObject] { return ChartUtils_Pie.bridgedObjCGetStringArray(swift: _extraLabels); }
    
    /// the legend colors array, each color is for the form drawn at the same index
    /// (ObjC bridging functions, as Swift 1.2 does not bridge optionals in array to NSNulls)
    public var colorsObjc: [NSObject]
        {
        get { return ChartUtils_Pie.bridgedObjCGetUIColorArray(swift: colors); }
        set { self.colors = ChartUtils_Pie.bridgedObjCGetUIColorArray(objc: newValue); }
    }
    ...
    public var labelsObjc: [NSObject]
        {
        get { return ChartUtils_Pie.bridgedObjCGetStringArray(swift: labels); }
        set { self.labels = ChartUtils_Pie.bridgedObjCGetStringArray(objc: newValue); }
    }
    ...
    public func setExtra(#colors: [NSObject], labels: [NSObject])
    {
        ...
        self._extraLabels = ChartUtils_Pie.bridgedObjCGetStringArray(objc: labels);
        self._extraColors = ChartUtils_Pie.bridgedObjCGetUIColorArray(objc: colors);
    }
