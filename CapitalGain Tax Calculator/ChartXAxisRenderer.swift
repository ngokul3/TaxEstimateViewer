    ...
    internal func drawLabels(#context: CGContext, pos: CGFloat)
    {
        ...
        for (var i = _minX, maxX = min(_maxX + 1, _xAxis.values.count); i < maxX; i += _xAxis.axisLabelModulus)
        {
            ...
            if (viewPortHandler.isInBoundsX(position.x))
            {
                ...
                ChartUtils_Pie.drawMultilineText(context: context, text: label!, point: CGPoint(x: position.x, y: pos), align: .Center, attributes: labelAttrs, constrainedToSize: labelMaxSize);
            }
        }
    }
