...
public class ChartYAxisRendererRadarChart: ChartYAxisRenderer
{
    ...
    internal override func computeAxisValues(min yMin: Double, max yMax: Double)
    {
        ...
        var interval = ChartUtils_Pie.roundToNextSignificant(number: Double(rawInterval))
        ...
        else
        {
            ...
            else
            {
                ...
                let last = ChartUtils_Pie.nextUp(floor(Double(yMax) / interval) * interval)
    ...
    public override func renderAxisLabels(context context: CGContext)
    {
        ...
        for (var j = 0; j < labelCount; j++)
        {
            ...
            let p = ChartUtils_Pie.getPosition(center: center, dist: r, angle: _chart.rotationAngle)
            
            let label = _yAxis.getFormattedLabel(j)
            
            ChartUtils_Pie.drawText(context: context, text: label, point: CGPoint(x: p.x + 10.0, y: p.y - labelLineHeight), align: .Left, attributes: [NSFontAttributeName: labelFont, NSForegroundColorAttributeName: labelTextColor])
        }
    }
    ...
    public override func renderLimitLines(context context: CGContext)
    {
        ...
        for (var i = 0; i < limitLines.count; i++)
        {
            ...
            {
                let p = ChartUtils_Pie.getPosition(center: center, dist: r, angle: sliceangle * CGFloat(j) + _chart.rotationAngle)
