...
public class RadarChartRenderer: ChartDataRendererBase
{
    ...
    internal func drawDataSet(#context: CGContext, dataSet: RadarChartDataSet)
    {
        ...
        for (var j = 0; j < entries.count; j++)
        {
            ...
            var p = ChartUtils_Pie.getPosition(center: center, dist: CGFloat(e.value - _chart.chartYMin) * factor, angle: sliceangle * CGFloat(j) + _chart.rotationAngle);
    ...
    public override func drawValues(#context: CGContext)
    {
        ...
        for (var i = 0, count = data.dataSetCount; i < count; i++)
        {
            ...
            for (var j = 0; j < entries.count; j++)
            {
                ...
                var p = ChartUtils_Pie.getPosition(center: center, dist: CGFloat(e.value) * factor, angle: sliceangle * CGFloat(j) + _chart.rotationAngle);
    ...
    internal func drawWeb(#context: CGContext)
    {
        ...
        {
            var p = ChartUtils_Pie.getPosition(center: center, dist: CGFloat(_chart.yRange) * factor, angle: sliceangle * CGFloat(i) + rotationangle);
        ...
        for (var j = 0; j < labelCount; j++)
        {
            for (var i = 0, xValCount = _chart.data!.xValCount; i < xValCount; i++)
            {
                ...
                var p1 = ChartUtils_Pie.getPosition(center: center, dist: r, angle: sliceangle * CGFloat(i) + rotationangle);
                var p2 = ChartUtils_Pie.getPosition(center: center, dist: r, angle: sliceangle * CGFloat(i + 1) + rotationangle);
    ...
    public override func drawHighlighted(#context: CGContext, indices: [ChartHighlight])
    {
        ...
        for (var i = 0; i < indices.count; i++)
        {
            ...
            var p = ChartUtils_Pie.getPosition(center: center, dist: CGFloat(y) * factor,
