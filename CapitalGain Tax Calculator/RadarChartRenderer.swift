...
public class RadarChartRenderer: LineScatterCandleRadarChartRenderer
{
    ...
    internal func drawDataSet(context context: CGContext, dataSet: RadarChartDataSet)
    {
        ...
        for (var j = 0; j < entries.count; j++)
        {
            ...
            let p = ChartUtils_Pie.getPosition(center: center, dist: CGFloat(e.value - _chart.chartYMin) * factor, angle: sliceangle * CGFloat(j) + _chart.rotationAngle)
    ...
    public override func drawValues(context context: CGContext)
    {
        ...
        for (var i = 0, count = data.dataSetCount; i < count; i++)
        {
            ...
            for (var j = 0; j < entries.count; j++)
            {
                ...
                let p = ChartUtils_Pie.getPosition(center: center, dist: CGFloat(e.value) * factor, angle: sliceangle * CGFloat(j) + _chart.rotationAngle)
                ...
                ChartUtils_Pie.drawText(context: context, text: formatter!.stringFromNumber(e.value)!, point: CGPoint(x: p.x, y: p.y - yoffset - valueFont.lineHeight), align: .Center, attributes: [NSFontAttributeName: valueFont, NSForegroundColorAttributeName: valueTextColor])
            }
        }
    }
    ...
    internal func drawWeb(context context: CGContext)
    {
        ...
        {
            let p = ChartUtils_Pie.getPosition(center: center, dist: CGFloat(_chart.yRange) * factor, angle: sliceangle * CGFloat(i) + rotationangle)
        ...
        for (var j = 0; j < labelCount; j++)
        {
            for (var i = 0, xValCount = _chart.data!.xValCount; i < xValCount; i++)
            {
                ...
                let p1 = ChartUtils_Pie.getPosition(center: center, dist: r, angle: sliceangle * CGFloat(i) + rotationangle)
                let p2 = ChartUtils_Pie.getPosition(center: center, dist: r, angle: sliceangle * CGFloat(i + 1) + rotationangle)
    ...
    public override func drawHighlighted(context context: CGContext, indices: [ChartHighlight])
    {
        ...
        for (var i = 0; i < indices.count; i++)
        {
            ...
            _highlightPointBuffer = ChartUtils_Pie.getPosition(center: center, dist: CGFloat(y) * factor,
                angle: sliceangle * CGFloat(j) + _chart.rotationAngle)
            
            // draw the lines
          //  drawHighlightLines(context: context, point: _highlightPointBuffer, set: set) //Convert
        }
